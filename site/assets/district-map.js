...
function buildDistrictMap(records){
 ...
 const content=detail.querySelector('.dialog-content');content.replaceChildren();const h=document.createElement('h2');h.textContent=d.district;content.append(h);const metro=(typeof metroParentLabel==='function'?metroParentLabel(d): (d.metro||''));const locBits=[d.state,metro||null,humanize(d.ai_policy_status)].filter(Boolean);const meta=document.createElement('p');meta.className='map-card-meta';meta.textContent=locBits.join(' · ');content.append(meta);if(typeof compositeScore==='function'){const c=compositeScore(d);if(c&&c.score!=null){const score=document.createElement('p');score.className='map-card-score';score.textContent=`Overall ${c.score} · ${c.label}`;content.append(score);}}const blurb=document.createElement('p');blurb.className='map-card-blurb';blurb.textContent=(typeof cardSummary==='function'?cardSummary(d):(d.one_liner||'No summary available.'));content.append(blurb);const safety=document.createElement('p');safety.className='map-card-safety';safety.textContent=`AI safety: ${(!d.safety_ai_status||d.safety_ai_status==='none'||d.safety_ai_status==='unknown')?'No public AI safety rule found':humanize(d.safety_ai_status)}`;content.append(safety);if(typeof entityHref==='function'){const lea=document.createElement('p');lea.className='map-card-cta';const href=entityHref(d);const id=(typeof slugify==='function'?slugify(d.district):href.split('/').filter(Boolean).pop());const a=document.createElement('a');a.href=href;a.textContent='Open full district page →';a.setAttribute('data-district-id',id);a.setAttribute('data-district-name',d.district||'');a.addEventListener('click',function(ev){if(window.AIPM_HOME&&typeof window.AIPM_HOME.handleDistrictOpen==='function'){window.AIPM_HOME.handleDistrictOpen(ev,id,href,d.district||'');}});lea.append(a);content.append(lea);}detail.showModal();}
 const points=[];
