...
function buildDistrictMap(records){
 ...
 const content=detail.querySelector('.dialog-content');content.replaceChildren();const h=document.createElement('h2');h.textContent=d.district;content.append(h);const metro=(typeof metroParentLabel==='function'?metroParentLabel(d): (d.metro||''));const locBits=[d.state,metro||null,humanize(d.ai_policy_status)].filter(Boolean);for(const text of [locBits.join(' · '),(typeof cardSummary==='function'?cardSummary(d):(d.one_liner||'No summary available.')),`AI safety: ${(!d.safety_ai_status||d.safety_ai_status==='none'||d.safety_ai_status==='unknown')?'No public AI safety rule found':humanize(d.safety_ai_status)}`]){const p=document.createElement('p');p.textContent=text;content.append(p);}if(typeof compositeScore==='function'){const c=compositeScore(d);if(c&&c.score!=null){const p=document.createElement('p');p.textContent=`Overall ${c.score} · ${c.label}`;content.append(p);}}if(typeof entityHref==='function'){const lea=document.createElement('p');const href=entityHref(d);const id=(typeof slugify==='function'?slugify(d.district):href.split('/').filter(Boolean).pop());const a=document.createElement('a');a.href=href;a.textContent='Open full district page →';a.setAttribute('data-district-id',id);a.setAttribute('data-district-name',d.district||'');a.addEventListener('click',function(ev){if(window.AIPM_HOME&&typeof window.AIPM_HOME.handleDistrictOpen==='function'){window.AIPM_HOME.handleDistrictOpen(ev,id,href,d.district||'');}});lea.append(a);content.append(lea);}detail.showModal();}
 const points=[];
